...
public:
    ...
    void Draw()
    {
        for (auto & i : body)
        {
            float x = i.x;
            float y = i.y;
            auto segment = Rectangle{(float) offset + x * (float) cellSize, (float ) offset + y * (float ) cellSize, (float)cellSize, (float)cellSize};
...
public:
    ...
    {
        auto x = GetRandomValue(0, cellCount - 1);
        auto y = GetRandomValue(0, cellCount - 1);
...
public:
    ...
    {
        InitAudioDevice();
        eatSound = LoadSound("../assets/sfx/eat.mp3");
        wallSound = LoadSound("../assets/sfx/wall.mp3");
    }
